	...
	processGet: function(response) {
		...
		if (rc !== 200) {
			...
			case 404: // not found.
				...
				break;
			case 504: // timeout.
				text += "接続がタイムアウトしました。ページの再読み込みをお試しください。<br />";
				break;
			default:
				text += "困りました。なぞのエラーです。<br />";
				break;
			}
			
			if (errors) {
				text += "response code=" + rc;
				for (var i = 0, size = errors.length; i < size; ++i) {
					text += "errors[" + i + "]=" + errors[i];
				}
			}
			$dev = $('<div style="color:#fff;background-color:#844;">' + text + '</div>');
		}
		else {
			text = response.text;
			$dev = $(text);
